...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_single_inclusion_BC_wavelets_orig", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is ###
saving_path = os.path.dirname(data_filepath) + '/train_runs_Silu_even_sampling'
