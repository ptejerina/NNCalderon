...
class ConductivityNetwork(nn.Module):
    ...
    """
    def __init__(self, ffe_dims, actv=nn.SiLU(), actv_last_layer=True, layers=4, neurons=64):
        """
...
############# UTILS ##################
    ...
    #     # Use epoch number in thousands to avoid crammed x-axis labels
    #     epochs_k = np.arange(1, len(tot_loss) + 1) / 1000.0

    #     fig, ax = plt.subplots(figsize=(10.5, 4.8), constrained_layout=True)

    #     ax.plot(epochs_k, tot_loss, label='Total', linewidth=2.0)
    #     ax.plot(epochs_k, de_loss, label='PDE', linewidth=1.6)
    #     ax.plot(epochs_k, f_bc_loss, label=r'Dirichlet $f_k$', linewidth=1.6)
    #     ax.plot(epochs_k, j_bc_loss, label=r'Neumann $J_k$', linewidth=1.6)
    ...
    #     # Put legend below the plot so it never gets cut off
    #     ax.legend(
    #         loc='upper center',
    ...
    #NEWWWW THIS AND FUNCTION BELOW. WRONG IT SEEMS
    # def _perimeter_order(self, coords, tol=1e-10):
    #     """
    #     Orders boundary points along the square perimeter:
    #     bottom -> right -> top -> left
    #     """
    #     x = coords[:, 0]
    #     y = coords[:, 1]

    #     s = np.full(coords.shape[0], np.nan, dtype=float)

    #     bottom = np.isclose(y, 0.0, atol=tol)
    #     right  = np.isclose(x, 1.0, atol=tol) & ~bottom
    #     top    = np.isclose(y, 1.0, atol=tol) & ~bottom & ~right
    #     left   = np.isclose(x, 0.0, atol=tol) & ~bottom & ~right & ~top

    #     s[bottom] = x[bottom]          # 0 -> 1
    #     s[right]  = 1.0 + y[right]     # 1 -> 2
    #     s[top]    = 3.0 - x[top]       # 2 -> 3
    #     s[left]   = 4.0 - y[left]      # 3 -> 4

    #     if np.isnan(s).any():
    #         bad = np.where(np.isnan(s))[0]
    #         raise ValueError(
    #             f"Some boundary points were not classified. Example bad indices: {bad[:10]}"
    #         )
    ...
    #     """
    #     Plot, for selected k:
    #     1) Dirichlet data f_k
    #     2) Current J_pred vs J_true
    #     3) Residual Delta J = J_pred - J_true
    #     """
    #     if isinstance(k_list, int):
    #         k_list = [k_list]

    #     coords = dataset.boundary_coords.detach().cpu().numpy()
    #     order, s_sorted = self._perimeter_order(coords)

    #     afine = np.arange(len(order))
    #     breaks = [
    #         np.searchsorted(s_sorted, 1.0, side='left'),
    #         np.searchsorted(s_sorted, 2.0, side='left'),
    #         np.searchsorted(s_sorted, 3.0, side='left'),
    #     ]
    ...
    #         f_pred = f_pred_t.detach().cpu().numpy().squeeze()
    #         J_pred = J_pred_t.detach().cpu().numpy().squeeze()

    #         f_true = dataset.boundary_potentials[k].detach().cpu().numpy().squeeze()
    #         J_true = dataset.currents[k].detach().cpu().numpy().squeeze()

    #         res = J_pred - J_true
    #         rms = np.sqrt(np.mean(res[order]**2))
    #         max_abs = np.max(np.abs(res[order]))

    #         fig, ax = plt.subplots(1, 3, figsize=(15.0, 3.2), constrained_layout=True)

    #         # (1) Dirichlet fit
    #         ax[0].plot(afine, f_true[order], label='true/measured', linewidth=1.8)
    #         ax[0].plot(afine, f_pred[order], linestyle='--', label='pred', linewidth=1.5)
    ...
    #         # (2) Current fit
    #         ax[1].plot(afine, J_true[order], label='measured', linewidth=1.8)
    #         ax[1].plot(afine, J_pred[order], linestyle='--', label='pred', linewidth=1.5)
    #         ax[1].set_title(f"Current $J_k$ (k={k})")
    ...
    #         for a in ax:
    #             for b in breaks:
    #                 a.axvline(b, color='k', linestyle='--', linewidth=0.5)
    #             a.grid(True, alpha=0.2)

    #         fig.suptitle(f"k={k} | RMS = {rms:.2e} | max |ΔJ| = {max_abs:.2e}")
    ...
    def plot_boundary_data(self, dataset, save_fig=False, show_fig=True):
        ...
        # Use ONE common boundary discretization/order for both true and predicted
        coords = dataset.boundary_coords.detach().cpu().numpy()
        N = int(coords.shape[0] / 4)
        ...
        # drop repeated end-corner of each side, exactly as on predicted side before
        perimeter_idx = np.concatenate([
            bottom_idx[:-1],
            right_idx[:-1],
            top_idx[:-1],
        ...
        for k in k_list:
            # True/measured values on the SAME ordered perimeter points
            f_true = f_all_true[k, perimeter_idx]
            J_true = J_all_true[k, perimeter_idx]

            # Predicted values
            _, f_pred, J_pred = self.plot_boundary_predictions(dataset, k_idx=k)
            f_nn = f_pred.detach().cpu().numpy().squeeze()[perimeter_idx]
            J_nn = J_pred.detach().cpu().numpy().squeeze()[perimeter_idx]

            fig, ax = plt.subplots(1, 2, figsize=(10, 2.4), constrained_layout=True)

            ax[0].plot(afine, f_nn, 'b-', label=r"NN $f(x,y)$")
            ax[0].plot(afine, f_true, 'b--', label=r"True $f(x,y)$")

            ax[1].plot(afine, J_nn, 'r-', label=r"NN $J(x,y)$")
            ax[1].plot(afine, J_true, 'r--', label=r"True $J(x,y)$")
            ...
            ax[0].set_xlim(left=0, right=len(perimeter_idx) - 1)
            ax[1].set_xlim(left=0, right=len(perimeter_idx) - 1)
            ...
            if save_fig is not False:
                if save_fig is True:
                    fig.savefig(
                        f'{self.path}/BC_f_and_J_recovered_k_{k}_epoch_{n_epochs}.pdf',
                        bbox_inches='tight'
                    )
                elif k in save_fig:
                    print('Saved plot', k)
    ...
    #         bnd_coords_np = bnd_coords.detach().cpu().numpy()
    ...
    #             print("\n========== DEBUG: boundary ordering comparison for k=40 ==========")
    #             print("true ordered coords shape:", bnd_coords_true[perimeter_idx_true].shape)
    #             print("pred ordered coords shape:", bnd_coords_np[perimeter_idx].shape)

    #             print(f"\nTRUE ordered coords [{s0}:{s1}]:")
    #             print(bnd_coords_true[perimeter_idx_true][s0:s1])

    #             print(f"\nPRED ordered coords [{s0}:{s1}]:")
    #             print(bnd_coords_np[perimeter_idx][s0:s1])

    #             print(f"\nTRUE f values [{s0}:{s1}]:")
    #             print(f_true[s0:s1])

    #             print(f"\nPRED f values [{s0}:{s1}]:")
    #             print(f_nn[s0:s1].squeeze())

    #             print(f"\nTRUE J values [{s0}:{s1}]:")
    #             print(J_true[s0:s1])

    #             print(f"\nPRED J values [{s0}:{s1}]:")
    #             print(J_nn[s0:s1].squeeze())
