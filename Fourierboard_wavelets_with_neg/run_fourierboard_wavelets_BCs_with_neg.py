...
CASES = ["fourierboard_wavelets_with_neg"]
...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_fourierboard_BC_wavelets_with_neg", "dtn_data_" + case + ".npz")
        print(data_filepath)
...
for i in range(reps):
    ...
    if i % 2 == 0 or i == reps - 1:
        trainer.plot_boundary_data(dataset=dataset, save_fig=[0,10,20,30,40,49,50,60,70,80], show_fig=False)
        plt.close()
        trainer.plot_induced_potential_u(save_fig=[0,10,20,30,40,49,50,60,70,80], show_fig=False)
        plt.close()
