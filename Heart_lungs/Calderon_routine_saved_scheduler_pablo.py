...
class IdentityEncoder:
    """
    Dummy encoder: returns raw coordinates unchanged.
    Used to run the model without Fourier feature encoding.
    """
...
class Trainer:
    ...
    def __init__(self, config, num_bcs, synthetic_data=None, saving_path = None, \
                 gamma_activ = nn.SiLU(), gamma_actv_last_layer = True, seed=None):
        """
        ...
        # Save seed to file
        with open(os.path.join(saving_path, "seed.txt"), "w") as f:
            f.write(str(seed))
        ...
        ####NEW TO ACCOUNT FOR NO FFE (RAW COORDS)
        ...
        if self.use_fourier_features:
            ...
            encoded_dim = config['ffe_mapping_size'] * 2
        else:
            self.ffe_encoder = IdentityEncoder(device=self.device)
            encoded_dim = 2

        self.gamma_net = ConductivityNetwork(
            ffe_dims=encoded_dim,
...
############# UTILS ##################
    ...
    def save_model(self, path):
        ...
        FFE_rand_matrix = self.ffe_encoder.B if self.use_fourier_features else None
    ...
    def load_model(self, path):
        ...
        if master_dict.get('use_fourier_features', self.use_fourier_features):
            if hasattr(self.ffe_encoder, 'B') and master_dict.get('FFE_rand_matrix') is not None:
    ...
    def plot_gamma_nn(self, N = 128, save_fig = False):
        ...
        # Absolute error between true gamma and NN gamma
        gamma_error = gamma_true - gamma_nn

        # MSE error
        mse_gamma = np.mean(gamma_error**2)

        # Optional extra metrics
        rmse_gamma = np.sqrt(mse_gamma)
        mae_gamma = np.mean(np.abs(gamma_error))
        max_abs_gamma_error = np.max(np.abs(gamma_error))

        # Relative error, same as before
        rel_error = np.log10(np.abs(gamma_error / (gamma_true + eps)))
        mean_rel_error = np.mean(10**rel_error)

        print('Mean Relative error:', mean_rel_error * 100, '%')
        print('MSE gamma error:', mse_gamma)
        print('RMSE gamma error:', rmse_gamma)


        # Save one gamma-error file per checkpoint epoch
        n_epochs = len(self.loss_history['pde'])

        error_txt_path = os.path.join(
            self.path,
            f"gamma_errors_epoch_{n_epochs}.txt"
        )

        with open(error_txt_path, "w") as f:
            f.write(f"epoch\t{n_epochs}\n")
            f.write(f"mse_gamma\t{mse_gamma:.8e}\n")
            f.write(f"rmse_gamma\t{rmse_gamma:.8e}\n")
            f.write(f"mae_gamma\t{mae_gamma:.8e}\n")
            f.write(f"max_abs_gamma_error\t{max_abs_gamma_error:.8e}\n")
            f.write(f"mean_relative_error_percent\t{mean_rel_error * 100:.8e}\n")

        print(f"Saved gamma errors to: {error_txt_path}")
