...
from Calderon_routine_saved_scheduler_pablo import *
...
CASES = ["horizontal_split_wavelets"]
...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_horizontal_split_wavelets", "dtn_data_" + case + ".npz")
        print(data_filepath)
...
saving_path = os.path.dirname(data_filepath) + f'/test_fixed_seed_{seed}'
print(saving_path)
...
trainer = Trainer(CONFIG, num_bcs=dataset.num_bcs, synthetic_data=data_filepath, saving_path=saving_path,\
                  gamma_activ = nn.SiLU(), gamma_actv_last_layer = True, seed = seed)
...
for i in range(reps):
    ...
    if i % 2 == 0 or i == reps - 1:
        ...
        plt.close()
        trainer.plot_boundary_fit(dataset=dataset, k_list=[0, 10, 20, 30, 40, 49], save_fig=True, show_fig=False)
        plt.close()
