...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "reduced_data_checkerboard_trig_wavelet_op_lin_parab_and_op_lin_ricker_54BC", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is ###
saving_path = os.path.dirname(data_filepath) + '/test_3_Relu_gamma_NO_actv_last_layer'
...
trainer = Trainer(CONFIG, num_bcs=dataset.num_bcs, synthetic_data=data_filepath, saving_path=saving_path,\
                  gamma_activ = nn.ReLU(), gamma_actv_last_layer = False)
