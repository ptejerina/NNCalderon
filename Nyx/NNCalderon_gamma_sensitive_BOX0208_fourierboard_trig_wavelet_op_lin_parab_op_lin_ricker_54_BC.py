...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "reduced_data_fourierboard_trig_wavelet_op_lin_parab_and_op_lin_ricker_54BC", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is ###
saving_path = os.path.dirname(data_filepath) + '/test_5_sensitive_gamma_BOX0208_samp'
...
CONFIG = {
    ...
    'sampling_method': 'square_dense_center',  # 'linear_dense_center', 'square_dense_center'
...
for i in range(reps):
    ...
    plt.close()
    trainer.compute_spatial_gamma_sensitivity(N=64, save_fig = True)
    plt.close()
