...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_checkerboard_trig_wavelet_opposite_linear_and_parabolic_70_BC", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is ###
saving_path = os.path.dirname(data_filepath) + '/test_3'
...
for i in range(reps):
    ...
    if i==0:
        trainer.train(dataset, case, noise_str, train_epochs=check_epochs, disable_progress_bar=True)
