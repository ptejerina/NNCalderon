...
from NNCalderon_rutine_orig import *
...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_checkerboard_63_MIX_BC", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is ###
saving_path = os.path.dirname(data_filepath) + '/test_1_pretrained_checkerboard_ADD_dipole_and_segment_BCs'
...
pretrained_model_path = f'{saving_path}/NN_epochs_98629'

trainer.load_model(path=pretrained_model_path)
...
for _ in range(reps):
    ...
    plt.close()
    trainer.plot_residuals(save_fig=[0,10,20,30,40,49], show_fig=False)
