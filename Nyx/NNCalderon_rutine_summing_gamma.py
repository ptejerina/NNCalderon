...
class ConductivityNetwork(nn.Module):
    def __init__(self, ffe_dims, num_bcs, actv=nn.SiLU(), actv_last_layer=True, layers=4, neurons=64):
        """
        Conductivity Network that can take BC index (one-hot) as additional input,
        but returns a single scalar gamma(x,y) independent of k by summing over k channels.
        """
        super().__init__()

        input_dim = ffe_dims + num_bcs  # include one-hot k
        net_layers = []
        ...
        # Instead of scalar output, output one gamma_k per BC
        net_layers.append(nn.Linear(neurons, num_bcs))
    ...
    def forward(self, x_ffe, k_one_hot):
        """
        Returns scalar gamma(x,y) that depends implicitly on k but is invariant under it.
        """
        net_input = torch.cat([x_ffe, k_one_hot], dim=-1)
        raw_output = self.network(net_input)  # shape (batch_size, num_bcs)

        # sum over k dimension to remove explicit dependence
        gamma_combined = raw_output.sum(dim=1, keepdim=True)
        gamma_normalized = torch.sigmoid(gamma_combined)  # [0,1] range
        gamma_scaled = self.min_gamma + (self.max_gamma - self.min_gamma) * gamma_normalized
        return gamma_scaled
...
class CalderonPINN:
    ...
    ################## ORIGINAL K-INDEPENDENT GAMMA FUNCTION computation #########################
    ...
    ######################### IMPLICIT DEPENDENCE OF GAMMA ON K #########################
    ...
    def compute_pde_residual(self, xy_colloc, k_indices):
        ...
        # gamma now depends implicitly on k
        gamma = self.gamma_net(xy_ffe, k_one_hot)
    ...
    def compute_data_predictions(self, xy_bnd, k_indices, normals):
        ...
        # Encode boundary condition indices
        k_one_hot = self._to_one_hot(k_indices, self.num_bcs)

        # Pass (x, y, k) to both networks
        gamma_pred = self.gamma_net(xy_ffe, k_one_hot)
...
class Trainer:
    ...
    def sample_dense_center(self, batch_size, device='cpu', density_factor=10, oversample_factor=10, multiply=True):
        ...
        Args:
            ...
            density_factor: how much denser the center should be vs. edges
            oversample_factor: multiplier for candidate pool size (higher = smoother)
            multiply: if True, multiplies x and y weights (stronger central focus);
                    if False, averages them (gentler density increase)
        """
        # Oversample uniformly
        N = batch_size * oversample_factor
        xy_uniform = torch.rand(N, 2, device=device)

        # Linear weights increasing toward center (1 at edges, density_factor at center)
    ...
    def sample_dense_center_square(self, batch_size, device='cpu', density_factor=10, center_bounds=(0.3, 0.7)):
        ...
        # 🔧 Adjust by either trimming or padding to match exactly batch_size
...
#################### ORIGINAL GAMMA PREDICTION (COMPLETE K INDEPEDENCE) #########################
    ...
    ############## PREDICT GAMMA WITH IMPLICIT K DEPENDENCE, WITH DUMMY INDEX ##############
    def predict_gamma(self, N=256):
        """Predicts the conductivity map on a uniform grid."""
        self.gamma_net.eval()
        x = torch.linspace(0, 1, N, device=self.device)
        ...
        with torch.no_grad():
            xy_ffe = self.ffe_encoder.encode(xy_grid)
            # Create a dummy one-hot vector for a single BC (e.g., k=0)
            k_one_hot = torch.zeros((xy_grid.shape[0], self.num_bcs), device=self.device)
            k_one_hot[:, 0] = 1.0  # use BC index 0
            gamma_pred = self.gamma_net(xy_ffe, k_one_hot)
...
############# UTILS ##################
    ...
    def plot_boundary_predictions(self, dataset, k_idx=0, use_gamma_th=False):
        ...
        if use_gamma_th==False:
            gamma_pred = gamma_net(xy_ffe, k_one_hot)
        else:
    ...
    def compute_spatial_gamma_sensitivity(self, N=64, save_fig = False):
        """
        Computes ∂γ/∂k along the active BC index for all BCs.
        Returns array of shape (num_bcs, N*N)
        """
        ...
        # Stack into (N*N, 2) list of coordinates
        xy_points = torch.stack([X.flatten(), Y.flatten()], dim=-1)
        xy_points.requires_grad_(True)
        # print('device', self.device)
        # print('xy_points', xy_points.device)

        xy_ffe = self.pinn_manager.ffe_encoder.encode(xy_points)

        sensitivities = []
        for k in range(self.pinn_manager.num_bcs):
            # Create a one-hot tensor for this BC
            k_tensor = torch.full((xy_points.size(0),), k, dtype=torch.long, device=self.pinn_manager.device)
            k_one_hot = self.pinn_manager._to_one_hot(k_tensor, self.pinn_manager.num_bcs)
            k_one_hot.requires_grad_(True)

            gamma = self.pinn_manager.gamma_net(xy_ffe, k_one_hot)  # shape (N*N,)

            # Compute gradient w.r.t. entire one-hot
            dgamma_dk_full = torch.autograd.grad(
                gamma, k_one_hot, torch.ones_like(gamma),
                retain_graph=True, create_graph=False
            )[0]  # shape (N*N, num_bcs)

            # Take only derivative along the active BC index
            dgamma_dk_active = dgamma_dk_full[:, k]  # shape (N*N,)

            sensitivities.append(dgamma_dk_active.detach().cpu().numpy())

        sensitivities = np.stack(sensitivities)  # shape (num_bcs, N*N)

        plot_step = 1  # plot all BCs
        ...
        perimeter_idx_true = np.concatenate([bottom_idx_true, right_idx_true, top_idx_true, left_idx_true])
        nb, nr, nt, nl = map(len, [bottom_idx_true, right_idx_true, top_idx_true, left_idx_true])

        ################### GRID OF BC SENSITIVITIES ###################
        cols = 5
        ...
        for k in range(K):
            ax = axs[k]
            f_true = f_all_true[k, perimeter_idx_true]
            sens_k = sensitivities[k, :].reshape(N_true, N_true)

            # Split f_true into four sides
            f_bottom = f_true[:nb]
            f_right  = f_true[nb:nb+nr]
            f_top    = f_true[nb+nr:nb+nr+nt]
            f_left   = f_true[nb+nr+nt:]

            # Normalize f for consistent color mapping
            f_norm = lambda f: (f - np.min(f_true)) / (np.max(f_true) - np.min(f_true))

            # Sensitivity map in log scale
            im = ax.imshow(
                np.abs(sens_k),
                origin='lower',
                extent=[0, 1, 0, 1],
                cmap='viridis',
                # norm=LogNorm(
                #     vmin=np.maximum(np.min(np.abs(sensitivities)), 1e-12),
                #     vmax=np.max(np.abs(sensitivities))
                # )
            )

            # Overlay boundary f_k profiles as color bars
            # Bottom
            x_b = np.linspace(0, 1, nb)
            ax.imshow(f_norm(f_bottom)[None, :],
                    extent=[0, 1, -0.05, 0],
                    aspect='auto', cmap='coolwarm', vmin=0, vmax=1)
            # Top
            x_t = np.linspace(0, 1, nt)
            ax.imshow(f_norm(f_top[::-1])[None, :],
                    extent=[0, 1, 1, 1.05],
                    aspect='auto', cmap='coolwarm', vmin=0, vmax=1)
            # Right
            y_r = np.linspace(0, 1, nr)
            ax.imshow(f_norm(f_right)[::-1, None],
                    extent=[1, 1.05, 0, 1],
                    aspect='auto', cmap='coolwarm', vmin=0, vmax=1)
            # Left
            y_l = np.linspace(0, 1, nl)
            ax.imshow(f_norm(f_left[::])[:, None],
                    extent=[-0.05, 0, 0, 1],
                    aspect='auto', cmap='coolwarm', vmin=0, vmax=1)
            ...
            ax.set_title(f"BC {k}", fontsize=9)

        # Hide any unused subplots
        for j in range(K, len(axs)):
            axs[j].axis('off')

        fig.suptitle("Sensitivity maps with boundary potentials (log scale)", fontsize=14)
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        fig.colorbar(im, ax=axs.tolist(), orientation='horizontal', fraction=0.02, pad=0.04, label=r'$|\partial \gamma / \partial k|$ (log scale)')
        if save_fig == True:
            fig.savefig(f'{self.path}/sensitivity_gamma_per_BC.pdf')
        plt.show()

        ################### MEAN SENSITIVITY MAP ###################
        fig2 = plt.figure(figsize=(6, 5))
        print(sensitivities.shape)
        av_sensitivity = np.mean(np.abs(sensitivities), axis=0).reshape(N_true, N_true)
        print(av_sensitivity.shape)
        ...
        im = plt.imshow(
            ...
            cmap='viridis',
            # norm=LogNorm(
            #     vmin=np.maximum(np.min(av_sensitivity), 1e-12),
            #     vmax=np.max(av_sensitivity)
            # )
        )
        cbar = plt.colorbar(im)
        cbar.set_label(r"Average $|\partial \gamma / \partial k|$", fontsize=12)

        plt.title("Log-scale Average Sensitivity Map", fontsize=13)
        plt.xlabel("x", fontsize=11)
        plt.ylabel("y", fontsize=11)
        plt.tight_layout()
        if save_fig == True:
            fig2.savefig(f'{self.path}/mean_sensitivity_gamma.pdf')
