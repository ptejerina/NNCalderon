...
CKPT_FILENAME = "gammaNN_DtN_steps_195050.pth"   # checkpoint file name (inside train_runs_DtN)
...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(
            current_dir,
            "data_single_inclusion_R4_BC_wavelets_orig",
        ...
        old_runs_dir = os.path.join(base_data_dir, "train_runs_DtN_full_batch")
        ckpt_path = os.path.join(old_runs_dir, CKPT_FILENAME)

        # ------------------------------------------------------------
        # >>> CHANGED: saving path is now in resumed_runs (new folder)
        # ------------------------------------------------------------
        saving_path = os.path.join(base_data_dir, "resumed_runs")
