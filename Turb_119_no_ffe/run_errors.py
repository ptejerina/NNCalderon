...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(
            current_dir,
            "data_single_inclusion_combined_190",
            "dtn_data_" + case + ".npz"
        )

        # Folder where the trained NN checkpoints are stored
        checkpoint_dir = os.path.dirname(data_filepath) + "/train_runs_cont'd"

        # Folder where you want to save gamma error txt files
        gamma_error_dir = os.path.dirname(data_filepath) + "/gamma_errors"
