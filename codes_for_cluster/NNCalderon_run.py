...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "mixture_data_checkerboard", "dtn_data_" + case + ".npz")
        print(data_filepath)



### Choose saving path: e.g. same path where data is. Can also choose new name (even of directory does not exists), it will be created ###

saving_path = os.path.dirname(data_filepath) + '/test_1'
# saving_path = 'Trial_for_cluster_run'
...
CONFIG = {
    ...
    'u_net_layers': 6,
    'u_net_neurons': 128,
    'loss_weights': {'pde': 1.0, 'dirichlet': 10.0, 'neumann': 10.0,'force_true_gamma': 0.0, 'tv': 1e-4}
}
...
plt.close()
trainer.plot_boundary_data(dataset=dataset, save_fig=[0,5,10,15,20,25,30])
plt.close()
trainer.plot_induced_potential_u(save_fig=[0,5,10,15,20,25,30])
plt.close()
trainer.plot_residuals(save_fig=[0, 5, 10, 15, 20,25, 30], show_fig=False)
