...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", "data_checkerboard_72_trig_wavelet_dipole_segment_BC", "dtn_data_" + case + ".npz")
        print(data_filepath)
...
selected_figs = list(np.arange(0,71,10))
...
for _ in range(reps):
    ...
    plt.close()
    trainer.plot_boundary_data(dataset=dataset, save_fig=selected_figs, show_fig=False)
    plt.close()
    trainer.plot_induced_potential_u(save_fig=selected_figs, show_fig=False)
    plt.close()
