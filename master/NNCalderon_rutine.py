...
# Uncomment below to initialize weights with Xavier uniform initialization 
# def init_weights(m):
#     """
...
# Define the Fourier Feature Encoder (FFE)
...
class CalderonPINN:
    ...
    ###### Forcing Gamma True Addloss - Not used for training ############
...
class CalderonDataset(Dataset):
    ...
    """
    def __init__(self, filepath, noise_level=0.0):
        """
...
class Trainer:
    ...
    def __init__(self, config, num_bcs, synthetic_data=None, saving_path = None, \
                 gamma_min = 0.5, gamma_max = 2.5, gamma_activ = nn.SiLU(), gamma_actv_last_layer = True, \
                 device = None):
        """
    ...
    ######## DEVICE IN WHICH TRAINING HAPPENS ############
        ...
        #### CHOOSE WHETHER THE RUTINE APPLIES FFE OR RAW COORDINATES (no FFE) to the network inputs ####
        ...
        # data = np.load(f"{os.getcwd()}/data_dir/dtn_data_gaussian_inclusion.npz")
    ...
    # Definition of other sampling rutines (linear_dense_center, square_dense_center) can be added here as needed.
    ...
    def train(self, dataset, case_name, noise_level_str=0.0, train_epochs = None, disable_progress_bar = True):
        ...
        for epoch in trange(1, epochs + 1, desc="Training epochs", disable=disable_progress_bar):
            ...
            for bnd_batch_cpu in dataloader:
                ...
                if self.config['sampling_method'] == 'uniform':
                    ...
                    if batch_number==0 and epoch==1:
                        print('xy_colloc.shape:', xy_colloc.shape)
                ...
                if len(self.loss_history['total'])==0 and epoch==1:
                ...
                #################### FORCING GAMMA TRUE AS SANITY CHECK (not used in paper) ################
            ...
            if epoch==1:
                print('num batches processed:', batch_number)



            # Plot one instance of the sampled points for visualization at the start of training
            if len(self.loss_history['total'])==0 and epoch==1:
            ...
            # Record the loss values for this epoch
