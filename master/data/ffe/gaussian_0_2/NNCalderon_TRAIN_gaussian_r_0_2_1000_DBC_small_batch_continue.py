...
for case in CASES:
    for noise_str, noise_val in NOISE_LEVELS.items():
        ...
        data_filepath = os.path.join(f"{current_dir}", f"data_gaussian_inclusion_140_BC_wavelets_radius_0.2", "dtn_data_" + case + ".npz")
        print(data_filepath)
...
saving_path = os.path.dirname(data_filepath) + f'/test_{test_number}_results_{case}_{num_BC}_BC_{BC_type}_seed_{seed}_samp_{CONFIG["sampling_method"]}_{CONFIG['loss_weights']['dirichlet_bc']}_DBC_small_batch'
...
for i in range(reps):
    ...
    print('Training time:' + str(elapsed_time/3600) + ' hours = ' + str(elapsed_time) + ' seconds' )
    ...
    elapsed_time = time.process_time() - t
    print('Saving time:' + str(elapsed_time) + ' seconds' )
